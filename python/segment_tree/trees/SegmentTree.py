...
class SegmentTree:
    ...
    def __init__(self, data):
        ...
        self.height_tree = ceil(log2(len(data)))
        self.mem_size = int(2 * 2 ** self.height_tree - 1)
        self.segment_tree = [0] * self.mem_size
        self._build_tree(0, self.len_data-1, 0)

    def _get_middle(self, s, e):
        return (e - s) // 2 + s
    ...
    '''
    def _build_tree(self, s, e, cur_index):
        if s == e:
            self.segment_tree[cur_index] = self.data[e]
            return self.data[e]
        middle_index = self._get_middle(s, e)
        '''
        ...
        '''
        l_value = self._build_tree(s, middle_index, cur_index * 2 + 1)
        r_value = self._build_tree(middle_index + 1, e, cur_index * 2 + 2)
    ...
    def _update(self, li, ls, index, value, cur_index):
        ...
        if li != ls:
            middle_index = self._get_middle(li, ls)
    ...
    def prefix_sum(self, i, j):
        if i < 0 or j > self.len_data - 1 or i > j:
            return -1
